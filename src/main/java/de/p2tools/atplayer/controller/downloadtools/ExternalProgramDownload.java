...
public class ExternalProgramDownload {
    ...
    private DownloadDirectHttp downloadDirectHttp;

    public ExternalProgramDownload(DownloadDirectHttp downloadDirectHttp, ProgData progData, DownloadData download) {
        super();
        this.downloadDirectHttp = downloadDirectHttp;
...
//        download.getDownloadStartDto().setProcess(runtimeExec.exec(true /* log */));
...
//
//            } else {
//                final int exitV = download.getDownloadStartDto().getProcess().exitValue(); //liefert ex wenn noch nicht fertig
//                if (exitV != 0) {
...
//                if (download.getDownloadStartDto().getStartCounter() < DownloadConstants.START_COUNTER_MAX) {
