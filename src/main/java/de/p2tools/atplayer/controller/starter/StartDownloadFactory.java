...
import de.p2tools.atplayer.controller.data.download.DownloadFactoryDelDownloadFiles;
import de.p2tools.atplayer.controller.data.downloaderror.DownloadErrorData;
import de.p2tools.atplayer.controller.downloadtools.ATInfoFile;
...
public class StartDownloadFactory {
    public static int SYSTEM_PARAMETER_DOWNLOAD_MAX_RESTART = 3;

    private StartDownloadFactory() {
    }

    public static void makeDirAndLoadInfoSubtitle(DownloadData download) {
        try {
            Files.createDirectories(Paths.get(download.getDestPath()));
            if (download.isInfoFile()) {
                ATInfoFile.writeInfoFile(download);
            }
    ...
    public static void finalizeDownload(DownloadData download) {
        ...
        ProgData.getInstance().downloadGuiController.refreshTable();
    ...
    public static boolean checkDownloadWasOK(ProgData progData, DownloadData download, StringProperty errorMsg) {
